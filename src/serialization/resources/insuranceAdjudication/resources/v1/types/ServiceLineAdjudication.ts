/**
 * This file was auto-generated by Fern from our API Definition.
 */

import * as serializers from "../../../../..";
import * as CandidApi from "../../../../../../api";
import * as core from "../../../../../../core";

export const ServiceLineAdjudication: core.serialization.ObjectSchema<
    serializers.insuranceAdjudication.v1.ServiceLineAdjudication.Raw,
    CandidApi.insuranceAdjudication.v1.ServiceLineAdjudication
> = core.serialization.object({
    serviceLineAdjudicationId: core.serialization.property(
        "service_line_adjudication_id",
        core.serialization.lazy(
            async () => (await import("../../../../..")).insuranceAdjudication.v1.ServiceLineAdjudicationId
        )
    ),
    denialReason: core.serialization.property(
        "denial_reason",
        core.serialization
            .lazy(async () => (await import("../../../../..")).serviceLines.v2.DenialReasonContent)
            .optional()
    ),
    insuranceAllowedAmountCents: core.serialization.property(
        "insurance_allowed_amount_cents",
        core.serialization.number().optional()
    ),
    insurancePaidAmountCents: core.serialization.property(
        "insurance_paid_amount_cents",
        core.serialization.number().optional()
    ),
    deductibleAmountCents: core.serialization.property(
        "deductible_amount_cents",
        core.serialization.number().optional()
    ),
    coinsuranceAmountCents: core.serialization.property(
        "coinsurance_amount_cents",
        core.serialization.number().optional()
    ),
    copayAmountCents: core.serialization.property("copay_amount_cents", core.serialization.number().optional()),
    carcs: core.serialization.list(
        core.serialization.lazyObject(
            async () => (await import("../../../../..")).insuranceAdjudication.v1.ClaimAdjustmentReasonCode
        )
    ),
    rarcs: core.serialization.list(
        core.serialization.lazyObject(
            async () => (await import("../../../../..")).insuranceAdjudication.v1.RemittanceAdviceRemarkCode
        )
    ),
});

export declare namespace ServiceLineAdjudication {
    interface Raw {
        service_line_adjudication_id: serializers.insuranceAdjudication.v1.ServiceLineAdjudicationId.Raw;
        denial_reason?: serializers.serviceLines.v2.DenialReasonContent.Raw | null;
        insurance_allowed_amount_cents?: number | null;
        insurance_paid_amount_cents?: number | null;
        deductible_amount_cents?: number | null;
        coinsurance_amount_cents?: number | null;
        copay_amount_cents?: number | null;
        carcs: serializers.insuranceAdjudication.v1.ClaimAdjustmentReasonCode.Raw[];
        rarcs: serializers.insuranceAdjudication.v1.RemittanceAdviceRemarkCode.Raw[];
    }
}
